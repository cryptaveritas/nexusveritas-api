...
function toV(bp) {
  ...
  return [
    ...
    1 / (1 + (b.avg_transfer_sol || 0)),          // [2] null-safe
    (b.split_init_pattern === true) ? 1 : 0,
    (s.funding_sources_count !== null && s.funding_sources_count !== undefined && s.funding_sources_count === 0) ? 1 : 0,
    Math.min(sigDensity / 100, 1.0),
    (b.recycling_loop === true) ? 1 : 0,
    1 / (1 + (s.wallet_age_days || 0) / 30),
    sigDensity > 50 ? 1 : sigDensity / 50,
    (o.total_signatures || 0) <= 20 ? 1 : 0,
    Math.min(Math.log1p(o.total_signatures || 0) / Math.log1p(20000), 1.0),
    ...
    ((b.recycling_loop === true ? 0.5 : 0) + (b.split_init_pattern === true ? 0.5 : 0)),
